"use client";

import SectionWrapper from "@/components/animations/SectionWrapper";
import RevealAnimation from "@/components/animations/RevealAnimation";

export default function SpecsTable() {
  const specs = [
    { component: "실시간 통신", tech: "WebRTC + LiveKit", desc: "P2P 기반 초저지연 오디오/비디오 스트리밍" },
    { component: "백엔드", tech: "Express.js", desc: "Node.js 기반 RESTful API 서버" },
    { component: "실시간 이벤트", tech: "Socket.io", desc: "양방향 실시간 통신 및 방 관리" },
    { component: "메시지 큐", tech: "RabbitMQ + Redis", desc: "비동기 작업 처리 및 캐싱" },
    { component: "AI 음원 분리", tech: "Mel-band Roformer", desc: "최신 음원 분리 AI — 보컬/MR 고품질 분리" },
    { component: "AI 가사 정렬", tech: "SOFA", desc: "강제 정렬 모델 — 음소 단위 가사 타이밍 생성" },
    { component: "AI 음정 분석", tech: "FCPE", desc: "실시간 F0 추출 — 빠르고 정확한 음정 분석" },
    { component: "AI Worker", tech: "Pika (RabbitMQ)", desc: "RabbitMQ 기반 GPU AI 작업 소비자" },
    { component: "스토리지", tech: "AWS S3", desc: "음원 및 미디어 파일 저장" },
    { component: "상태 관리", tech: "Redux", desc: "클라이언트 전역 상태 관리" },
    { component: "모니터링", tech: "ELK Stack", desc: "로그 수집, 분석, 시각화" },
  ];

  return (
    <SectionWrapper id="specs" className="w-full py-16 sm:py-24 md:py-32 text-white px-6 md:px-20">
      <div className="max-w-7xl mx-auto">
        <RevealAnimation>
          <h2 className="mb-8 sm:mb-12 md:mb-16 text-xs font-bold tracking-[0.2em] text-white/50 uppercase">
            Technical Specification
          </h2>
        </RevealAnimation>
        
        <div className="w-full overflow-x-auto">
          <table className="w-full min-w-[600px] text-left border-collapse">
            <thead>
              <tr className="border-b border-white/20">
                <th className="py-4 sm:py-6 text-sm font-medium text-white/60">Component</th>
                <th className="py-4 sm:py-6 text-sm font-medium text-white/60">Technology</th>
                <th className="py-4 sm:py-6 text-sm font-medium text-white/60">Description</th>
              </tr>
            </thead>
            <tbody>
              {specs.map((row, i) => (
                <RevealAnimation 
                  as="tr" 
                  key={i} 
                  delay={i * 0.05} 
                  className="border-b border-white/10 transition-colors hover:bg-white/5"
                >
                  <td className="py-4 pr-4 sm:py-6 sm:pr-8 font-medium text-white">{row.component}</td>
                  <td className="py-4 pr-4 sm:py-6 sm:pr-8 font-mono text-[#C0C0C0]">{row.tech}</td>
                  <td className="py-4 sm:py-6 text-gray-400">{row.desc}</td>
                </RevealAnimation>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </SectionWrapper>
  );
}
